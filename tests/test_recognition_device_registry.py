...
class RecognitionDeviceRegistryTests(unittest.TestCase):
    ...
    def test_registry_registers_only_explicit_bindings_and_preserves_history(self):
        registry = RecognitionDeviceRegistry()
        change = registry.register(binding())
        registry.activate("bind.camera", "receipt.activate.1")
        self.assertIsNone(change.previous_state)
        self.assertEqual(registry.get("bind.camera").state, DeviceBindingState.ACTIVE)
        self.assertEqual(registry.by_device("usb.camera.1").binding_id, "bind.camera")
        self.assertEqual(len(registry.history()), 2)
        self.assertTrue(all(not item.authority_granted for item in registry.history()))

    def test_registry_rejects_duplicate_device_module_and_binding_ids(self):
        registry = RecognitionDeviceRegistry()
    ...
    def test_storage_scanning_and_device_self_registration_are_forbidden(self):
        registry = RecognitionDeviceRegistry()
        with self.assertRaisesRegex(RuntimeError, "scanning are forbidden"):
    ...
    def test_modality_filter_returns_only_active_matching_bindings(self):
        ...
        self.assertEqual(
            registry.active_bindings(RecognitionModality.IMAGE),
            (registry.get(camera.binding_id),),
        )
    ...
    def test_simulated_binding_remains_explicitly_marked(self):
        item = binding(simulated=True)
        self.assertTrue(item.simulated)
        self.assertEqual(item.configuration["frame_id"], "vehicle-cabin")

    def test_binding_rejects_unknown_adapter_and_authority_claims(self):
        with self.assertRaisesRegex(ValueError, "unsupported adapter_kind"):
            binding(adapter_kind="mystery")
        with self.assertRaisesRegex(ValueError, "cannot claim authority"):
            RecognitionDeviceBinding(
                binding_id="bind.bad",
